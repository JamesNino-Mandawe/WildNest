...
ORDER BY r.CreatedAt DESC;");
        ...
        private void ShowConfirmThenCheckIn(string reservationId)
        {
            ...
            // QR codes now encode a file:// URL pointing to the guest's
            // self-contained HTML Booking Pass (Option B upgrade).
            // Extract the booking ID from the filename so the rest of the
            // flow works identically whether the QR is old (plain ID) or new.
            //
            //   file:///C:/Users/.../BookingPasses/WN-2026-0001.html
            //   → "WN-2026-0001"
            if (reservationId.StartsWith("file://", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    string localPath = new Uri(reservationId).LocalPath;
                    reservationId    = Path.GetFileNameWithoutExtension(localPath);
                }
                catch
                {
                    // Malformed URL — let it fall through; the DB lookup will reject it cleanly
                }
            }
