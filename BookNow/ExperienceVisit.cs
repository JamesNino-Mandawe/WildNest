...
namespace Project.Booking
{
    public partial class ExperienceVisit : UserControl
    {
        ...
        private void BuildUi()
        {
            ...
            if (_uiBuilt)
                ResetGeneratedLayout(new[] { pnlExperiences, pnlSchedule, pnlGuestInfo, pnlPayment });
            ...
            BuildGuestStep();
            BuildPaymentStep();
            ShowStep(Math.Max(1, Math.Min(4, stepToShow)));
            FireSummary();
        }
        ...
        private void BuildExperiencesStep()
        {
            ...
            _dtVisitDate.ValueChanged += (s, e) => FireSummary();
            ...
            _cboArrival.SelectedIndexChanged += (s, e) => FireSummary();
        ...
        private void BuildScheduleStep()
        {
            ...
            {
                ("8:00 AM","Morning session","Available"),
                ("11:00 AM","Midday session","Available"),
                ("3:00 PM","Afternoon session","Available"),
            };
        ...
        private void BuildPaymentStep()
        {
            ...
            secPay.Height = 72 + payBody.Height + 32;
        ...
        private void ConfirmBooking()
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Please select at least one experience.", "Experience required",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ShowStep(1);
                    return;
                }

                int guestCount = Math.Max(1, _adults + _children);
                decimal total = CalculateExperienceTotal();

                var rCmd = new MySqlCommand(@"
            INSERT INTO tbl_Reservations
                (ReservationID, GuestID, BookingType, VisitDate, NumAdults, NumChildren, TotalAmount, ArrivalTime)
            VALUES (@rid, @gid, 'ExperienceVisit', @vd, @adults, @children, @total, @arrival);", conn, tx);
                ...
                rCmd.Parameters.AddWithValue("@arrival", GetSelectedSlot() ?? _cboArrival?.Text ?? "Front Desk Confirmation");
                rCmd.ExecuteNonQuery();

                foreach (var experience in SelectedExperiences())
                {
                    int? experienceId = BookingPersistence.ResolveExperienceId(conn, tx, experience.Name);
                    if (experienceId.HasValue)
                        BookingPersistence.InsertExperienceLink(conn, tx, bid, experienceId.Value, guestCount);
                }
                ...
                _ = System.Threading.Tasks.Task.Run(() =>
                {
                    var result = EmailService.SendConfirmation(
                        ...
                        bid, "Experience Visit", $"Visit Date: {_dtVisitDate.Value:MMM dd, yyyy}", total, _paymentMethod, (System.Drawing.Bitmap)_qrBitmap.Clone());
                    if (!result.Success)
                        ProjectDiagnostics.LogWarning("ExperienceVisit", $"Booking {bid} was saved but confirmation email failed: {result.Message}");
                });
        ...
        {
            int experienceTotal = CalculateExperienceSubtotal();
            var addons = new List<Project.AddonItem>();
            int guests = Math.Max(1, _adults + _children);

            foreach (var experience in SelectedExperiences())
            {
                addons.Add(new Project.AddonItem { Name = experience.Name, Price = experience.Price * guests });
            }

            string selectedSlot = "Schedule pending";
            foreach (var slot in _slotCards)
            {
                if (slot.Tag is bool selected && selected)
                {
                    selectedSlot = slot.AccessibleName ?? selectedSlot;
                    break;
                }
            }
            ...
            OnSummaryChanged?.Invoke(new Project.BookingSummary
            {
                BookingType = "Experience Only",
                CabinName = addons.Count > 0 ? $"{addons.Count} Experience{(addons.Count == 1 ? "" : "s")}" : "Choose Experiences",
                PrimarySubtitle = $"{_dtVisitDate?.Value:MMM dd, yyyy} · {selectedSlot}",
                PrimaryAmountLabel = "/booking",
                PrimaryAmount = experienceTotal,
                GrandTotal = CalculateExperienceTotal(),
                Addons = addons,
                Lines = new List<Project.SummaryLine>
                {
                    new() { Label = "Adults", Value = _adults.ToString() },
                    new() { Label = "Children", Value = _children.ToString() },
                    new() { Label = "Guests", Value = guests.ToString() },
                    new() { Label = "Arrival", Value = _cboArrival?.Text ?? "Not selected" },
                    new() { Label = "Fees", Value = "₱400" }
                }
            });
        }
        ...
        private string? GetSelectedSlot()
        {
            ...
            {
                if (slot.Tag is bool selected && selected)
                    return slot.AccessibleName;
            }

            return null;
        }

        private int CalculateExperienceSubtotal()
        {
            int guestCount = Math.Max(1, _adults + _children);
            int subtotal = 0;

            foreach (var experience in SelectedExperiences())
                subtotal += experience.Price * guestCount;

            return subtotal;
        }

        private int CalculateExperienceTotal() => CalculateExperienceSubtotal() + 400;

        private List<(string Name, int Price)> SelectedExperiences()
        {
            var selectedExperiences = new List<(string Name, int Price)>();
            foreach (var card in _expCards)
            {
                if (card.Tag is not bool selected || !selected)
                    continue;

                string[] parts = (card.AccessibleName ?? "").Split('|');
                string name = parts.Length > 0 ? parts[0] : "Wildlife experience";
                int price = parts.Length > 1 && int.TryParse(parts[1], out int parsed) ? parsed : 0;
                selectedExperiences.Add((name, price));
            }
