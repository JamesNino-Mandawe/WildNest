...
namespace Project.Booking
{
    public partial class DayVisit : UserControl
    {
        ...
        private void BuildUi()
        {
            ...
            if (_uiBuilt)
                ResetGeneratedLayout(new[] { pnlVisitDetails, pnlGuestInfo, pnlPayment, pnlConfirm });
        ...
        private void BuildVisitDetailsStep()
        {
            ...
            _cboEntryTime.SelectedIndexChanged += (s, e) => FireSummary();
            ...
            zBody.Height = zBodyH;
            sZones.Height = 72 + zBody.Height + 32;
            ...
            for (int i = 0; i < addons.Length; i++)
            {
                ...
                ac.AccessibleName = $"{aname}|{aprice}";
                ac.Paint += (ss, ee) =>
                {
                ...
                EventHandler toggle2 = (ss, ee) => { ac.Tag = !(bool)ac.Tag; ac.Invalidate(); FireSummary(); };
                ac.Click += toggle2;
            ...
            sAddons.Height = 72 + aBody.Height + 32;
        ...
        private void BuildGuestStep()
        {
            ...
            cont.Location = new Point(W - cont.Width, 8); cont.Click += (s, e) => { if (EnsureGuestVerificationReady()) ShowStep(3); };
        ...
        private void ConfirmBooking()
        {
            ...
            try
            {
                ...
                decimal total = CalculateDayVisitTotal();
            ...
            VALUES (@rid, @gid, 'DayVisit', @vd, @adults, @children, @total, @arrival);", conn, tx);
                ...
                rCmd.Parameters.AddWithValue("@arrival", _cboEntryTime?.Text ?? "Front Desk Confirmation");
                rCmd.ExecuteNonQuery();

                int guestCount = Math.Max(1, _adults + _children + _seniors);
                foreach (var addon in SelectedAddons())
                {
                    int? experienceId = BookingPersistence.ResolveExperienceId(conn, tx, addon.Name);
                ...
                _ = System.Threading.Tasks.Task.Run(() =>
                {
                    var result = EmailService.SendConfirmation(
                        ...
                        bid, "Day Visit", $"Visit Date: {_dtVisitDate.Value:MMM dd, yyyy}", total, _paymentMethod, (System.Drawing.Bitmap)_qrBitmap.Clone());
                    if (!result.Success)
                        ProjectDiagnostics.LogWarning("DayVisit", $"Booking {bid} was saved but confirmation email failed: {result.Message}");
                });
        ...
        private Panel CounterCard(string title, string subtitle, int x, int w, ref int count, ref Label lbl)
        {
            ...
            btnM.Click += (s, e) =>
            {
                ...
                SyncVisitorCounter(title, localCount);
            };
            btnP.Click += (s, e) =>
            {
                localCount++;
        ...
        private void SyncVisitorCounter(string title, int value)
        {
            if (title.StartsWith("Adults", StringComparison.OrdinalIgnoreCase))
                _adults = value;
            else if (title.StartsWith("Children", StringComparison.OrdinalIgnoreCase))
                _children = value;
            else
                _seniors = value;
        ...
        private int CalculateDayVisitTotal()
        {
            int baseTotal = (_adults * 450) + (_children * 250) + (_seniors * 350);
            int guestCount = Math.Max(1, _adults + _children + _seniors);
            int addonTotal = 0;
            ...
            return baseTotal + addonTotal + 200;
        }

        private List<(string Name, int Price)> SelectedAddons()
        {
            var selectedAddons = new List<(string Name, int Price)>();
            foreach (var card in _addonCards)
            {
                ...
                string name = parts.Length > 0 ? parts[0] : "Experience add-on";
                int price = parts.Length > 1 && int.TryParse(parts[1], out int parsed) ? parsed : 0;
                selectedAddons.Add((name, price));
            }
        ...
        {
            int visitorTotal = (_adults * 450) + (_children * 250) + (_seniors * 350);
            var addons = new List<Project.AddonItem>();

            int guestCount = Math.Max(1, _adults + _children + _seniors);
            foreach (var addon in SelectedAddons())
            {
                addons.Add(new Project.AddonItem { Name = addon.Name, Price = addon.Price * guestCount });
            }

            int selectedZones = 0;
            foreach (var card in _zoneCards)
                if (card.Tag is bool selected && selected)
                    selectedZones++;
            ...
            OnSummaryChanged?.Invoke(new Project.BookingSummary
            {
                ...
                CabinName = "Day Pass",
                PrimarySubtitle = $"{_dtVisitDate?.Value:MMM dd, yyyy} · {_cboEntryTime?.Text ?? "Entry time"}",
                PrimaryAmountLabel = "/visit",
                PrimaryAmount = visitorTotal,
                GrandTotal = CalculateDayVisitTotal(),
                Addons = addons,
                Lines = new List<Project.SummaryLine>
                {
                    new() { Label = "Adults", Value = _adults.ToString() },
                    new() { Label = "Children", Value = _children.ToString() },
                    new() { Label = "Seniors / PWD", Value = _seniors.ToString() },
                    new() { Label = "Zones selected", Value = selectedZones > 0 ? $"{selectedZones} zones" : "Custom pending" },
