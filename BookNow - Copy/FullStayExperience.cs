...
namespace Project.Booking
{
    public partial class FullStayExperience : UserControl
    {
        ...
        private void BuildStayStep()
        {
            ...
            int ccW = (W - GAP) / 2, ccH = 80;

            for (int i = 0; i < cabins.Length; i++)
            {
                var (name, meta, price, status, icon) = cabins[i];
                bool booked = status == "Booked";
                Color stClr = status == "Available" ? BookingFlowTheme.Success : status == "Last room" ? BookingFlowTheme.Warning : BookingFlowTheme.Danger;
        ...
        private void ConfirmBooking()
        {
            ...
            INSERT INTO tbl_Guests (FirstName, LastName, Email, Phone, Nationality, ValidIDType, SpecialRequests)
            VALUES (@fn, @ln, @em, @ph, @nat, @id, @req);", conn);
                ...
                gCmd.Parameters.AddWithValue("@req", _txtRequests?.Text.Trim() ?? "");
            ...
            VALUES (@rid, @gid, @cid, 'FullStay',
                ...
                foreach (var ec in _expCards.Where(c => (bool)c.Tag))
                {
                    ...
                    if (!string.IsNullOrEmpty(expName))
                    {
                        ...
                        expCmd.Parameters.AddWithValue("@n", expName);
        ...
        private void RefreshReview()
        {
            ...
            int selectedExps = _expCards.Count(ec => (bool)ec.Tag);
        ...
        private Panel CounterCard(string title, string subtitle, int x, int w, ref int count, ref Label lbl, Action? onChange)
        {
            ...
            btnM.Click += (s, e) => { localCount = Math.Max(0, localCount - 1); lblCount.Text = localCount.ToString(); onChange?.Invoke(); FireSummary(); };
            btnP.Click += (s, e) => { localCount++; lblCount.Text = localCount.ToString(); onChange?.Invoke(); FireSummary(); };
