...
namespace Project
{
    public partial class UcCabin : UserControl
    {
        ...
        private void BuildUI()
        {
            ...
            lblHeroTitle.Size = new Size(860, 88);
        ...
        private void SetupNewUI()
        {
            ...
            pnlBody.Height = 860; // initial; updated dynamically
        ...
        private void DoGridLayout(string filter)
        {
            ...
            int minSidebarBodyH = btnFilterVilla.Bottom + 20;
            pnlBody.Height = Math.Max(pnlMain.Height, minSidebarBodyH);
            pnlSideIcons.Height = pnlBody.Height;
            this.Height = pnlHero.Height + pnlStatusBar.Height + pnlBody.Height;
        ...
        private void SetupFilterButtons()
        {
            var filters = new (Button btn, string emoji, string label, string filter)[]
            {
                ...
                (btnFilterTreehouse, "🌳", "Treehouse", "Treehouse"),
                (btnFilterForest,    "🌲", "Forest", "Forest Cabin"),
                (btnFilterGlamping,  "⛺", "Glamping", "Glamping Tent"),
                (btnFilterLodge,     "🏠", "Lodge",  "Lodge"),
                (btnFilterEco,       "🌿", "Eco Stay", "Eco Bungalow"),
        ...
        private void PaintFilterBtn(PaintEventArgs e, Button btn, string emoji, string label, bool active)
        {
            ...
            if (label == "All")
            {
                var glyphRect = new Rectangle(orbRect.X + (orbRect.Width - 22) / 2, orbRect.Y + 7, 22, 18);
                using (var pen = new Pen(active ? C_GOLD : Color.FromArgb(185, C_CREAM), 1.6f))
                using (var fill = new SolidBrush(Color.FromArgb(active ? 46 : 20, active ? C_GOLD : C_CREAM)))
                {
                    for (int row = 0; row < 2; row++)
                    {
                        for (int col = 0; col < 2; col++)
                        {
                            var cell = new Rectangle(glyphRect.X + col * 12, glyphRect.Y + row * 10, 9, 7);
                            using (var cellPath = RoundRect(cell, 2))
                            {
            ...
            {
                using (var emojiBrush = new SolidBrush(active ? C_GOLD : Color.FromArgb(185, C_CREAM)))
        ...
        private void PaintCabinHeroTitle(object sender, PaintEventArgs e)
        {
            ...
            using var font = new Font("Georgia", 52f, FontStyle.Bold);
            ...
            float total = leftSize.Width + rightSize.Width + 16f;
        ...
        private void PaintSignature(object sender, PaintEventArgs e)
        {
            ...
            // Price
            TextRenderer.DrawText(g, _signature.Price, new Font("Georgia", 22f, FontStyle.Bold),
                new Rectangle(rx, iy - 2, 190, 38), C_GOLD_LIGHT,
                TextFormatFlags.Left | TextFormatFlags.VerticalCenter);
            TextRenderer.DrawText(g, "/night", new Font("Segoe UI", 7.5f),
                new Rectangle(rx, iy + 28, 70, 14), Color.FromArgb(90, C_CREAM), TextFormatFlags.Left);
        ...
        private void ShowDetail(CabinInfo cabin)
        {
            ...
            Control host = parentForm as Control ?? this;

            int mw = Math.Min(800, host.Width - 72);
            int mh = Math.Min(670, host.Height - 64);
            ...
            // ── hero
            var hero = new Panel { Dock = DockStyle.Top, Height = 174, BackColor = Color.Transparent };
            hero.Paint += (s, pe) =>
            {
                var g = pe.Graphics;
                g.SmoothingMode = SmoothingMode.AntiAlias;
                ...
                using (var br = new LinearGradientBrush(hr, cabin.BgTop, cabin.BgBot, 130f))
                    g.FillPath(br, path);
                using (var shimmer = new LinearGradientBrush(new Rectangle(0, 0, hr.Width, 3),
                    Color.Transparent, Color.FromArgb(170, C_GOLD_LIGHT), 0f))
                ...
                TextRenderer.DrawText(g, cabin.Icon, new Font("Segoe UI Emoji", 52f), iconRect, Color.White,
                    TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
                ...
                using (var br = new SolidBrush(Color.FromArgb(215, 14, 80, 52))) g.FillPath(br, path);
            ...
            // ── bottom strip
            var strip = new Panel { Dock = DockStyle.Bottom, Height = 84, BackColor = Color.FromArgb(8, 22, 13) };
            strip.Paint += (s, pe) =>
            {
                pe.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
                ...
                TextRenderer.DrawText(pe.Graphics, cabin.Price, new Font("Georgia", 18f, FontStyle.Bold),
                    new Rectangle(28, 10, 180, 36), C_GOLD_LIGHT,
                    TextFormatFlags.Left | TextFormatFlags.VerticalCenter);
                TextRenderer.DrawText(pe.Graphics, "/night", new Font("Segoe UI", 8.5f),
                    new Rectangle(28, 48, 72, 18), Color.FromArgb(160, C_CREAM), TextFormatFlags.Left);
                var noteRect = new Rectangle(96, 46, 162, 24);
                ...
                TextRenderer.DrawText(pe.Graphics, cabin.Cancel, new Font("Segoe UI", 8f, FontStyle.Bold),
                    noteRect, Color.FromArgb(210, 236, 224),
                    TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
            };
            ...
            {
                SizeF titleSize = g2.MeasureString(cabin.Name, nameLabel.Font, contentW);
            ...
            // gold divider
            body.Controls.Add(new Panel { Height = 2, Width = 50, BackColor = C_GOLD, Location = new Point(contentX + (contentW - 50) / 2, by) });
            by += 14;
            ...
            using (var g2 = body.CreateGraphics())
            {
                SizeF sz = g2.MeasureString(cabin.Desc, descLabel.Font, contentW);
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                chip.Controls.Add(new Label { Text = iLabels[i], Font = new Font("Segoe UI", 7.5f), ForeColor = Color.FromArgb(120, C_GOLD), AutoSize = false, Size = new Size(cellW - 16, 16), BackColor = Color.Transparent, Location = new Point(10, 5), AutoEllipsis = true, TextAlign = ContentAlignment.MiddleCenter });
                chip.Controls.Add(new Label { Text = iVals[i], Font = new Font("Segoe UI", 8.75f, FontStyle.Bold), ForeColor = C_CREAM, AutoSize = false, Size = new Size(cellW - 16, 20), BackColor = Color.Transparent, Location = new Point(10, 22), AutoEllipsis = true, TextAlign = ContentAlignment.MiddleCenter });
            ...
            int aChipW = (contentW - 8) / 3, aChipH = 76;
            for (int ai = 0; ai < cabin.Amenities.Length; ai++)
            {
                ...
                chip.Controls.Add(new Label { Text = icon, Font = new Font("Segoe UI Emoji", 17f), AutoSize = false, Size = new Size(aChipW, 30), BackColor = Color.Transparent, Location = new Point(0, 8), TextAlign = ContentAlignment.MiddleCenter });
                chip.Controls.Add(new Label { Text = iname, Font = new Font("Segoe UI", 7.25f), ForeColor = Color.FromArgb(208, C_CREAM), AutoSize = false, Size = new Size(aChipW - 10, 30), BackColor = Color.Transparent, Location = new Point(5, 40), TextAlign = ContentAlignment.TopCenter, AutoEllipsis = true });
                body.Controls.Add(chip);
            }
        }
        ...
        private void PaintHeroBackground(object sender, PaintEventArgs pe)
        {
            ...
            HeroSurfacePainter.Paint(pe.Graphics, pnl.ClientRectangle, HeroSurfaceVariant.Cabins);
        }
    }
}
